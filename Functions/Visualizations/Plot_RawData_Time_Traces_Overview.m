...
if ~isfield(params,'rlimits'),params.rlimits=[10,40];end
if ~isfield(params, 'yscale'),params.yscale = 'log';
end
...
for j=1:Nwl
    ...
    if istablevar(info.pairs,'lambda')
        title(['All ',num2str(info.pairs.lambda(j)),' nm, Rsd:',...
            num2str(params.rlimits(1,1)),'-',...
            num2str(params.rlimits(1,2)),' mm'],...
            'Color','w')
    else
        title(['All WL ## ',num2str(info.pairs.WL(j)),' nm, Rsd:',...
            num2str(params.rlimits(1,1)),'-',...
            num2str(params.rlimits(1,2)),' mm'],...
            'Color','w')
    end
...
for j=1:Nwl
    ...
    xlabel('Time [sec]');ylabel('\Phi')
    if istablevar(info.pairs,'lambda')
        title(['Good ',num2str(info.pairs.lambda(j)),' nm, Rsd:',...
            num2str(params.rlimits(1,1)),'-',...
            num2str(params.rlimits(1,2)),' mm'],...
            'Color','w')
    else
        title(['Good WL ## ',num2str(info.pairs.WL(j)),' nm, Rsd:',...
            num2str(params.rlimits(1,1)),'-',...
            num2str(params.rlimits(1,2)),' mm'],...
            'Color','w')
    end    
